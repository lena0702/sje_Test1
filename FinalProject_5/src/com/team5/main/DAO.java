...
public class DAO {
	...
	public static void regUser(HttpServletRequest request) {
		...
		try {
			...
			if (ss.insert("team5.regUser", mu) == 1) {
				...
				request.setAttribute("signInResult", "등록 성공");
			} else {
				request.setAttribute("signInResult", "등록 실패");
			}
			ss.commit();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static List<Review> getAllReviews(HttpServletRequest request) {
		...
		try {
			...
			request.setAttribute("reviewsNum", allReviews.size());
			return allReviews;
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public static List<Review> getMyReviews(HttpServletRequest request) {
		...
		if (mu != null) {
			...
			try {
				SqlSession ss = DBManager.connect();
				List<Review> reviews = ss.selectList("team5.getMyReview", mu);
